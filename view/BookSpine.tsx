...
import { resolveSpineImagePath } from '../services/imageStorage';
...
export const BookSpine = memo(
  ({ book, width: providedWidth, height: providedHeight }: BookSpineProps) => {
    // Memoize calculations to avoid recalculating on every render
    const { bookWidthPx, actualBookHeight, fontSize, backgroundColor } =
      useMemo(() => {
    ...
    return (
      <View
        ...
        {book.spineThumbnail ? (
          <Image
            source={{ uri: resolveSpineImagePath(book.spineThumbnail) }}
            style={[
              styles.spineImage,
              {
                width: bookWidthPx,
