...
interface BookshelfOverlayProps {
  ...
  onShelfChange?: (shelfIndex: number) => void;
}
...
export function BookshelfOverlay({
  ...
  // Calculate shelf index for each book
  const bookShelfIndices = useMemo(() => {
    let currentX = 0;
    let currentShelf = 0;
    ...
    for (const book of booksWithDimensions) {
      ...
      indices.push(currentShelf);
      currentX += book.bookWidth + bookGap;
    }
    return indices;
  }, [booksWithDimensions, bookGap]);

  // Notify when the selected book's shelf changes
  useEffect(() => {
    if (selectedBookId && onShelfChange) {
      const index = books.findIndex(b => b.id === selectedBookId);
      if (index >= 0 && bookShelfIndices[index] !== undefined) {
        onShelfChange(bookShelfIndices[index]);
      }
    }
  }, [selectedBookId, bookShelfIndices, books, onShelfChange]);
  ...
  const bookScales = useMemo(() => {
    ...
    if (scrollProgress === undefined) {
      if (selectedBookId) {
        ...
        return books.map(book => {
          return selectedBookId === book.id ? 1.08 : 1.0;
        });
      }
    ...
    return books.map((_, index) => {
      ...
      // Scale from 1.0 (far) to 1.08 (selected)
      ...
      const scale = 1.0 + 0.08 * scaleFactor;
